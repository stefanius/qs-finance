...
class ExcelKolom 
{
	...
	function dorun($data){
            ...
            $baseRow = 5;
            $total=0;
            $objPHPExcel->getActiveSheet()->setCellValue('A1', 'Balans: '.$data['beginbalans']['Bookyear']['omschrijving']);
            /*
            foreach($data['beginbalans']['debet']['posten'] as $r => $dataRow) {
                $row = $baseRow + $i;
                $total=$row;
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$total, $data['beginbalans']['credit']['totaal']);
            ...
            foreach($data['beginbalans']['credit']['posten'] as $r => $dataRow) {
                $row = $baseRow + $i;
                $total=$row;
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$total, $data['beginbalans']['credit']['totaal']);
            $objPHPExcel->getActiveSheet()->removeRow($baseRow-1,1);		
*/
            $downloader = new Download($objReader, $objPHPExcel);
            $downloader->download('kolombalans_'.$data['beginbalans']['Bookyear']['omschrijving']);
    }
}
?>
